...
requireMethod('POST');
rateLimit('send-email-verification', 5, 900);
...
try {
    ...
    // Only resend if there is a pending (not-yet-confirmed) registration
    $s = $pdo->prepare('SELECT id, first_name, total_attempts FROM pending_registrations WHERE email = ? LIMIT 1');
    $s->execute([$email]);
    $row = $s->fetch();

    if (!$row) {
        jsonResponse(['success' => false, 'message' => 'No pending registration found for this email.']);
    }

    // Block resend if the hard total limit was already reached
    ...
    // Reset per-OTP attempts but keep total_attempts intact
    $pdo->prepare(
        'UPDATE pending_registrations
            SET otp = ?, attempts = 0, expires_at = NOW() + INTERVAL 5 MINUTE
          WHERE id = ?'
    )->execute([$otp, $row['id']]);

    sendVerificationEmail($email, $otp, $row['first_name']);
...
function sendVerificationEmail(string $to, string $otp, string $firstName = ''): void {
    ...
    $mail->addAddress($to);
    $mail->isHTML(true);
    $mail->Subject = 'Your New Opticana Verification Code';
    $mail->Body    = verificationEmailBody($otp, $firstName);
    $mail->AltBody = "Your new Opticana verification code is: $otp\n\nExpires in 5 minutes.";
    $mail->send();
}
        ...
        <tr>
          <td style="padding:40px 40px 0;text-align:center;">
            ...
            <p style="margin:0 auto;font-size:14px;color:#6b7280;line-height:1.7;max-width:360px;">
              {$greeting} Here is your new verification code. Valid for
