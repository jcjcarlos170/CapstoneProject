...
//
//  POST { action:'profile', firstName, middleName, lastName, phone, email, address, occupation }
//
...
try {
    ...
    if ($action === 'profile') {
        ...
        $pdo->prepare(
            'UPDATE patients SET first_name = ?, middle_name = ?, last_name = ?, contact = ?, address = ?, occupation = ? WHERE user_id = ?'
        )->execute([$fn, $mn ?: null, $ln, $phone, $address, $occupation, $userId]);
